...
| Stateful navigation and compound rows | Inspect Navigation Menu open versus hover, including muted descendant contrast; Tabs active versus inactive; Command Input boundaries, search-row padding, group-heading separation, item gaps, row height, and leading-icon size; and SidebarMenuButton + portaled DropdownMenu geometry using the primitive's actual data attributes. Navigation Menu and Select chevrons use a zero-line-height 16px frame and 16px glyph, centered with the text and immune to broad SVG sizing. Line Tabs have no list/trigger border or shadow and use only a safely inset indicator that stays outside rounded ends. |
...
| Button Group | Inspect horizontal, vertical, mixed, and nested groups as one frame. Children have no standalone radius/border/shadow, adjacent items share one separator, and focus/pressed states do not shift or double the outline. The outer shadow matches the theme token and reference depth; a flat theme stays flat. |
...
| Calendar and card footer | Inspect calendar cells at 100% inside the actual gallery canvas; keep default cells at least 40px and no smaller than the normal control height. Month/year dropdown captions use control-sized hit areas and locale-safe widths. Custom days with secondary text use taller non-square rows with visible internal breathing room. Selected single days and range endpoints retain their fill and focus follows the cell shape. Check that Card footer buttons and icons sit vertically centered with balanced top/bottom padding, including small and wrapped variants. |
| Icons | Consistent family, optical size and stroke; align to the label rather than the SVG box alone. Bound Button icons explicitly and scale compact-button icons down. Verify the Input OTP Resend Code icon does not dominate its label. Quiet action icons should not compete with illustration. |
...
Treat the gallery sidebar header as a composition check too. Review the account/version switcher and search control together. Require a clear vertical gap, aligned edges, balanced header padding, and no accidental overlap or crowding at Desktop and in the Mobile drawer.
