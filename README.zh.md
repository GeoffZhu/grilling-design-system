...
分区域拆解内容与风格 → 建立图片特征到实现的对应表 → 规划视觉决策树 → 整体风格 → 基础规则 → 组件与状态 → 对照原图检查 → 电脑和手机整合预览 → 修改或确认 → 完整交付。

- 先看内容层级、构图、色彩占比、字号对比、形状组合、疏密、材质与图像用法，区分直接观察和推断。具体方法见 [visual-analysis.md](skills/grilling-design-system/references/visual-analysis.md)。
- 保留图中同时存在的不同处理方式，例如醒目的重点内容、平静的表单和紧凑的辅助列表。每个重要特征对应到 token、组件变体、自定义组件、素材或整合构图，不能只记录在文字里。
- 生成后对照原图检查层级、轮廓、配色分布和内容组合，并检查普通组件是否保留这些规则。重点特征的改写、缺失和素材限制需要说明。
