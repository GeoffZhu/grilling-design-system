...
Use meaningful custom APIs with controlled/uncontrolled state where useful. Do not bake example content into the component library. Show image-derived components in core and full galleries. Stickers may be original SVG/CSS approximations; identify them as interpretations, not extracted originals. Bundle required assets and licenses or state intentional limitations.
