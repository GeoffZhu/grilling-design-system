...
- Components: retain the required usage rules and preserve official shadcn APIs. Explain primary/default and md/default mappings. Add image-derived components within this section when needed.
...
- Implementation, Reference Screens, and Canonical Components: use existing paths relative to the directory containing DESIGN_DOC. In Web projects these are relative to the application root, even when the component destination is elsewhere. Inspect custom/shared components and reference screens. Mark absent screen types as not applicable with a reason; never fabricate paths or duplicate components to fill a field.
...
Before regenerating, preserve the completed DESIGN_DOC with the normal project backup. In integrated mode, the renderer writes PROJECT/DESIGN.draft.md when root DESIGN.md already exists; merge it into the root document. Keep durable guidance in design-notes.md or IMAGE-COMPONENTS.md; the generator imports or links these under section 13. Reapply project-specific values and translations after regeneration, checking them against current tokens and code.
